...
export default function AthletesReportPage() {
    ...
    const handleBulkImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const response = await fetch('http://localhost:3001/api/athletes/bulk-import', {
                method: 'POST',
                body: formData,
                credentials: 'include'
            });
    ...
    const downloadTemplate = () => {
        const csvContent = "name,gender,category,age,schoolId,personalBest\nJohn Doe,MALE,U17,15,school-uuid-here,11.2s";
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-neutral-800 pb-6 gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <button
                        onClick={async () => {
                            try {
                                const response = await fetch('http://localhost:3001/api/admin/export?type=athletes&format=csv', {
                                    credentials: 'include'
                                });
                                ...
                                const a = document.createElement('a');
                                a.href = url;
                                a.download = `athletes-report-${new Date().toISOString().split('T')[0]}.csv`;
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-neutral-900 border border-neutral-800 rounded-[2.5rem] w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-8 border-b border-neutral-800 flex justify-between items-center">
                            <div>
                                <h3 className="text-2xl font-black italic text-white uppercase tracking-tighter">Bulk Import Athletes</h3>
                                <p className="text-xs font-mono text-neutral-500 uppercase tracking-widest mt-1">Upload CSV Data</p>
                            </div>
                            <button onClick={() => { setIsImportModalOpen(false); setImportResults(null); }} className="text-neutral-500 hover:text-white transition-colors">
                                    ...
                                    <div className="space-y-4">
                                        <p className="text-neutral-400 text-sm">
                                            ...
                                            <code className="block bg-black p-3 rounded-xl mt-2 text-blue-500 font-mono text-xs">
                                                name, gender, category, age, schoolId, personalBest
                                            </code>
                                        </p>
